...
fn walk_liberty(ren_id:i16, color:i8, pos:&mut Position, target:usize){
    ...
    if pos.board.get(target) == 0 && pos.ren_id_board.get(target) != ren_id + 1000 {
        pos.liberty_count_map.add(ren_id as usize, 1);
    }
    
    if (
        // 連IDが振られてたら終了。ただし「自分の連ID + 1000」を除く。0 は枠セル番号なんで、連IDに使わない。
        pos.ren_id_board.get(target) != 0 && pos.ren_id_board.get(target) != ren_id + 1000)
        || // または、
        // 探している石でなければ終了。
        pos.board.get(target) != color
    {
        ...
        // 1000以上の連ID --> 同上。
        if pos.board.get(target) == 0 || 1000 <= pos.ren_id_board.get(target) {
            pos.ren_id_board.set(target, ren_id + 1000);
        }
        return;
    }
...
fn walk_empty(ren_id:usize, pos:&mut Position, target:usize) {
    match pos.board.get(target) {
        0 => {
            // 空点。
            if 0 < pos.empty_ren_id_board.get(target) {
