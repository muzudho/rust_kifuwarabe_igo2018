...
pub fn walk_empty(ren_id:usize, pos:&mut Position, target:usize) {
    match pos.board.get(target) {
        ...
        1 => {
            if pos.get_ren_database().get_territory().get_owner(ren_id) == 3 {
                // 占有者は白黒両方の場合、更新なし。
            } else if pos.get_ren_database().get_territory().get_owner(ren_id) == 2 {
                // 黒石 かつ、占有者が 黒、白の両方になる。
                pos.get_mut_ren_database().get_mut_territory().set_owner(ren_id, 3);
            } else {
                // 占有者は黒石か。
                pos.get_mut_ren_database().get_mut_territory().set_owner(ren_id, 1);
            }
        ...
        2 => {
            if pos.get_ren_database().get_territory().get_owner(ren_id) == 3 {
                // 占有者は白黒両方の場合、更新なし。
            } else if pos.get_ren_database().get_territory().get_owner(ren_id) == 1 {
                // 白石 かつ、占有者が 黒、白の両方になる。
                pos.get_mut_ren_database().get_mut_territory().set_owner(ren_id, 3);
            } else {
                // 占有者は白石か。
                pos.get_mut_ren_database().get_mut_territory().set_owner(ren_id, 2);
            }
